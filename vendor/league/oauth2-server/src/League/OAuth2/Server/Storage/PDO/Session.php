...
class Session implements SessionInterface
{
    ...
    public function validateAccessToken($accessToken)
    {
        ...
        //Addedd access_token inorder to check with user input accesstoken
		$stmt = $db->prepare('SELECT session_id, oauth_sessions.`client_id`, oauth_sessions.`owner_id`, oauth_sessions.`owner_type`,o.`access_token` FROM `oauth_session_access_tokens` as o JOIN oauth_sessions ON oauth_sessions.`id` = session_id WHERE access_token = :accessToken order by o.id desc limit 0,1');// AND access_token_expires >= ' . time()
        $stmt->bindValue(':accessToken', $accessToken);
        $stmt->execute();
