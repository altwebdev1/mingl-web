...
class Password implements GrantTypeInterface {
    ...
    /**
     * Callback to authenticate a user's email , password ,facebook and linkedin id
     * @var function
     */
    ...
    public function completeFlow($inputParams = null)
    {
		...
		if (isset($inputParams['SilentLogin']) && $inputParams['SilentLogin'] == 1) {
			$silentLogin = 1;
			unset($inputParams['SilentLogin']);
		}
		
        // Get the required params
        $authParams = $this->authServer->getParam(array('ClientId', 'ClientSecret', 'FacebookId', 'LinkedInId','Email','Password','DeviceToken','EndPointARN','Platform'), 'post', $inputParams);
	    ...
	    $authParams['client_details'] = $clientDetails; // broadtags login check
		
			$flag = 0;
			if(!is_null($authParams['Email']) && !is_null($authParams['Password'])){
				$flag = 1;
			}
			if ($flag == 0 && is_null($authParams['FacebookId']) &&  is_null($authParams['LinkedInId'])) {
	            throw new Exception\ClientException(sprintf($this->authServer->getExceptionMessage('invalid_request'), 'FacebookId'), 0);
	        }
			
	        // Check if user's facebookid or linkedin are correct
	         $userInfo = call_user_func($this->getVerifyCredentialsCallback(), $authParams['Email'], $authParams['Password'], $authParams['FacebookId'], $authParams['LinkedInId'], $authParams['DeviceToken'], $authParams['EndPointARN'], $authParams['Platform']);
        ...
        $sessionId = $this->authServer->getStorage('session')->createSession($authParams['ClientId'], 'user', $userInfo['UserId']);
		...
		if ($silentLogin == 0) {
			...
			//$response_array['meta'] = array("code" =>"201","dataPropertyName"=> "login");
			$response_array['Registration'] = $response;
		}
